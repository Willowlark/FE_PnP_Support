...
class Unit(CsvImport):
    ...
    #Name,Class,Lv,Exp,HP,Str,Mag,Skl,Spd,Luck,Def,Res,Con,Mov,Affinity,I1,,,,,,,,Usable Weapons,
...
def save(command):
    ...
    for key in scope:
        value = scope[key]
        # if type(value) is Unit and units: writer.writerow(value.writeString())
        # if type(value) is Item and items: writer.writerow(value.writeString())
        # if type(value) is Enemy and enemies: writer.writerow(value.writeString())
        ...
        try:
            ...
            if 'fights' in command:
                ...
                if " ".join(command) in scope:
                    ...
                    print "Unrecognized command, object not in scope."
        except KeyError:
